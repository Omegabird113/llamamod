...
import mc.omegabird.llamablocks.world.inventory.*;
import mc.omegabird.llamablocks.network.MenuStateUpdateMessage;
import mc.omegabird.llamablocks.LlamablocksMod;

import java.util.Map;

public class LlamablocksModMenus {
	public static final DeferredRegister<MenuType<?>> REGISTRY = DeferredRegister.create(Registries.MENU, LlamablocksMod.MODID);
	...
	public interface MenuAccessor {
		...
		default void sendMenuStateUpdate(Player player, int elementType, String name, Object elementState, boolean needClientUpdate) {
			...
			if (player instanceof ServerPlayer serverPlayer) {
				...
				if (Minecraft.getInstance().screen instanceof LlamablocksModScreens.ScreenAccessor accessor && needClientUpdate)
