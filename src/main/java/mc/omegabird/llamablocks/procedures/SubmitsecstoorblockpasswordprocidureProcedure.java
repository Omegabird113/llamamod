...
public class SubmitsecstoorblockpasswordprocidureProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (((entity instanceof Player _entity0 && _entity0.containerMenu instanceof LlamablocksModMenus.MenuAccessor _menu0) ? _menu0.getMenuState(0, "password", "") : "")
			...
			LlamablocksMod.LOGGER.debug(("Secure Storage Block at (" + x + ", " + y + ", " + z + ") authenticated successfully."));
		} else {
			if (entity instanceof ServerPlayer _ent) {
				BlockPos _bpos = BlockPos.containing(x, y, z);
				_ent.openMenu(new MenuProvider() {
			...
			LlamablocksMod.LOGGER.debug(("Secure Storage Block at (" + x + ", " + y + ", " + z + ") FAILED to authenticate."));
		}
	}
