...
public class BannanaplantOnBoneMealSuccessProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		y_offest = 1;
		while ((world.getBlockState(BlockPos.containing(x, y + y_offest, z))).getBlock() == LlamablocksModBlocks.BANANA_PLANT.get()) {
			y_offest = y_offest + 1;
		}
		if ((world.getBlockState(BlockPos.containing(x, y + y_offest, z))).getBlock() == Blocks.AIR) {
			world.setBlock(BlockPos.containing(x, y + y_offest, z), LlamablocksModBlocks.BANANA_PLANT.get().defaultBlockState(), 3);
			if ((world.getBlockState(BlockPos.containing(x, y + y_offest + 1, z))).getBlock() == Blocks.AIR && totalheight != 4) {
				world.setBlock(BlockPos.containing(x, y + y_offest + 1, z), LlamablocksModBlocks.BANANA_PLANT.get().defaultBlockState(), 3);
			}
		}
	}
}