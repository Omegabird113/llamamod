...
import net.mcreator.llamamod.procedures.SubmitsecstoorblockpasswordprocidureProcedure;
...
public record SecurestooragepasswordguiButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {
	...
	}, (RegistryFriendlyByteBuf buffer) -> new SecurestooragepasswordguiButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt()));
	@Override
	public Type<SecurestooragepasswordguiButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final SecurestooragepasswordguiButtonMessage message, final IPayloadContext context) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		if (buttonID == 0) {
			...
			SubmitsecstoorblockpasswordprocidureProcedure.execute(world, x, y, z, entity);
		}
		if (buttonID == 1) {
