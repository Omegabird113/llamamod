...
import net.mcreator.llamamod.network.AuthblockauthguiButtonMessage;
...
public class AuthblockauthguiScreen extends AbstractContainerScreen<AuthblockauthguiMenu> implements LlamaModModScreens.ScreenAccessor {
	...
	private static final ResourceLocation texture = ResourceLocation.parse("llama_mod:textures/screens/authblockauthgui.png");
	...
	protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int mouseX, int mouseY) {
		...
		guiGraphics.blit(RenderPipelines.GUI_TEXTURED, ResourceLocation.parse("llama_mod:textures/screens/my-levae_the_password_feild_empty_if_you_havent_yet_configured_a_password..png"), this.leftPos + -15, this.topPos + 76, 0, 0, 0, 0, 0, 0);
	}
	...
	protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		guiGraphics.drawString(this.font, Component.translatable("gui.llama_mod.authblockauthgui.label_secure_stoorage_block"), 5, 4, -16777216, false);
		guiGraphics.drawString(this.font, Component.translatable("gui.llama_mod.authblockauthgui.label_enter_password"), 5, 15, -16777016, false);
	}

	@Override
	public void init() {
		super.init();
		password = new EditBox(this.font, this.leftPos + 6, this.topPos + 26, 118, 18, Component.translatable("gui.llama_mod.authblockauthgui.password"));
		password.setMaxLength(8192);
		password.setResponder(content -> {
			if (!menuStateUpdateActive)
				menu.sendMenuStateUpdate(entity, 0, "password", content, false);
		});
		password.setHint(Component.translatable("gui.llama_mod.authblockauthgui.password"));
		this.addWidget(this.password);
		button_submit = Button.builder(Component.translatable("gui.llama_mod.authblockauthgui.button_submit"), e -> {
		...
		button_x = Button.builder(Component.translatable("gui.llama_mod.authblockauthgui.button_x"), e -> {
			int x = AuthblockauthguiScreen.this.x;
			int y = AuthblockauthguiScreen.this.y;
			if (true) {
