...
import net.mcreator.llamamod.world.inventory.PasswordchangerguiMenu;
import net.mcreator.llamamod.procedures.PasswordbananaprivlidgeescheckProcedure;
import net.mcreator.llamamod.procedures.PasswordbananaRightclickedOnBlockProcedure;
import net.mcreator.llamamod.network.PasswordchangerguiButtonMessage;
...
public class PasswordchangerguiScreen extends AbstractContainerScreen<PasswordchangerguiMenu> implements LlamaModModScreens.ScreenAccessor {
	...
	private static final ResourceLocation texture = ResourceLocation.parse("llama_mod:textures/screens/passwordchangergui.png");
	...
	protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		guiGraphics.drawString(this.font, Component.translatable("gui.llama_mod.passwordchangergui.label_secure_stoorage_block"), 4, 4, -16777216, false);
		guiGraphics.drawString(this.font, Component.translatable("gui.llama_mod.passwordchangergui.label_enter_password"), 4, 13, -16777016, false);
		guiGraphics.drawString(this.font, Component.translatable("gui.llama_mod.passwordchangergui.label_enter_password_again"), 4, 42, -16777131, false);
		guiGraphics.drawString(this.font, Component.translatable("gui.llama_mod.passwordchangergui.label_if_there_is_one"), 4, 50, -16777131, false);
		if (PasswordbananaprivlidgeescheckProcedure.execute(entity))
			guiGraphics.drawString(this.font, Component.translatable("gui.llama_mod.passwordchangergui.label_change_password"), 1, 107, -1, false);
		if (PasswordbananaprivlidgeescheckProcedure.execute(entity))
	...
	public void init() {
		super.init();
		password = new EditBox(this.font, this.leftPos + 5, this.topPos + 23, 118, 18, Component.translatable("gui.llama_mod.passwordchangergui.password"));
		password.setMaxLength(8192);
		password.setResponder(content -> {
			if (!menuStateUpdateActive)
				menu.sendMenuStateUpdate(entity, 0, "password", content, false);
		});
		password.setHint(Component.translatable("gui.llama_mod.passwordchangergui.password"));
		this.addWidget(this.password);
		current_password = new EditBox(this.font, this.leftPos + 5, this.topPos + 61, 118, 18, Component.translatable("gui.llama_mod.passwordchangergui.current_password"));
		...
		current_password.setHint(Component.translatable("gui.llama_mod.passwordchangergui.current_password"));
		this.addWidget(this.current_password);
		button_submit = Button.builder(Component.translatable("gui.llama_mod.passwordchangergui.button_submit"), e -> {
		...
		button_x = Button.builder(Component.translatable("gui.llama_mod.passwordchangergui.button_x"), e -> {
			int x = PasswordchangerguiScreen.this.x;
			int y = PasswordchangerguiScreen.this.y;
			if (true) {
		...
		button_reset = Button.builder(Component.translatable("gui.llama_mod.passwordchangergui.button_reset"), e -> {
			int x = PasswordchangerguiScreen.this.x;
			int y = PasswordchangerguiScreen.this.y;
			if (PasswordbananaprivlidgeescheckProcedure.execute(entity)) {
